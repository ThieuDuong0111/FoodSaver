...
abstract class CartCheckoutRemoteDataSource {
  Future<Either<Failure, CartEntity>> cartCheckOut();
}

@LazySingleton(as: CartCheckoutRemoteDataSource)
class CartCheckoutRemoteDataSourceImpl implements CartCheckoutRemoteDataSource {
  CartCheckoutRemoteDataSourceImpl(
    this._appStorage,
    this._appHttpClient,
  ...
  @override
  Future<Either<Failure, CartEntity>> cartCheckOut() async {
    try {
      ...
      final Response response = await _appHttpClient.post(
        Uri.parse(
          '${ApiEndpoints.baseUrl}/cart/checkout',
        ),
