...
abstract class ProductByStoreRemoteDataSource {
  Future<Either<Failure, List<ProductEntity>>> getProductByStore(int storeId);
}

@LazySingleton(as: ProductByStoreRemoteDataSource)
class ProductByStoreRemoteDataSourceImpl implements ProductByStoreRemoteDataSource {
  ProductByStoreRemoteDataSourceImpl(
    this._appHttpClient,
  ...
  @override
  Future<Either<Failure, List<ProductEntity>>> getProductByStore(int storeId) async {
    try {
      final Response response = await _appHttpClient.get(
        Uri.parse(
          '${ApiEndpoints.baseUrl}/products/by-store/$storeId',
        ),
